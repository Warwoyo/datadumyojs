# OJS 3.4.0-7 Realistic Instance Seed (`ojs34-realistic-instance-v1`)

A deterministic, resettable seed that injects **realistic dummy data** into an
OJS 3.4.0-7 install so the instance looks like a journal that is **running
...
```bash
APP_DIR=/var/www/ojs-340 FILES_DIR=/var/ojs-files \
...
`mysql/database.sql` is a **data-only seed** (INSERTs) for OJS 3.4.0-7. It does
**not** create tables and does **not** touch OJS source code. Apply it on top of
a freshly installed OJS 3.4.0-7 schema (the installer creates the tables and the
default `site` row). The seed is regenerated from the same `fixture-spec.json` so
the database IDs always match the file inventory.

Because the journal row is inserted directly (not created through the OJS UI),
the seed **also inserts the 12 default submission-file genres** for the journal
context, which OJS would otherwise auto-create. The only foreign-key-relevant
default assumed already present on a fresh install is the `site` row.

### OJS 3.4 schema specifics handled by the generator
Verified against the official PKP dataset (`pkp/datasets`, `ojs/stable-3_4_0`):

| Area | 3.3 | 3.4 (this seed) |
|---|---|---|
| Locale codes | `en_US` | `en` (`varchar(14)`) |
| List settings / `users.locales` | PHP `serialize()` | JSON (`["en"]`) |
| Current issue | `issues.current` | `journals.current_issue_id` |
| Submission state | `submission_progress` int | `submissions.status` + `submission_progress` varchar (`''`) |
| `publications.date_published` | datetime | `DATE` (`YYYY-MM-DD`) |
| Genres | assumed present | seeded explicitly per context |

The on-disk layout (`journals/<jid>/articles/<submission_id>/<file>`) is the same
as 3.4; OJS reads each file from its `files.path` value, so the deterministic,
human-readable physical filenames used here work identically to OJS's own
random-hex names.
...
3.4.0-7 install look like a healthy, normally-operating journal populated with
