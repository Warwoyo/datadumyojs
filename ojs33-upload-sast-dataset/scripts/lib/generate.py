#!/usr/bin/env python3
"""
Deterministic generator for a REALISTIC, normally-operating OJS 3.3.0-13 seed.

This package injects dummy-but-realistic data into an OJS 3.3.0-13 install: a
...
ROOT = Path(__file__).resolve().parents[2]   # ojs33-upload-sast-dataset/
...
def write_sql(spec, file_records):
    ...
    L = []
    A = L.append
    A("-- OJS 3.3.0-13 realistic instance seed (data-only).")
    A("-- dataset_id: %s" % spec["dataset_id"])
    A("-- Generated %s. Benign content only; no executable uploads." % NOW_UTC)
    A("--")
    A("-- SCOPE: data-only seed. Apply on top of a FRESH OJS 3.3.0-13 schema")
    A("--        (default site row, default genres genre_id=1) created by the")
    A("--        OJS installer. It does NOT create tables and does NOT modify")
    A("--        OJS source. See README.md > 'Applying the database'.")
    A("SET NAMES utf8mb4;")
    ...
    for iss in spec["issues"]:
        issue_rows.append({
            ...
            "published": iss["published"], "current": iss["current"],
    ...
    A(insert("issues",
             ["issue_id", "journal_id", "volume", "number", "year", "published",
              "current", "date_published", "access_status", "show_volume",
    ...
    for sub in spec["submissions"]:
        ...
        sub_rows.append({
            ...
            "submission_progress": 0 if sub["status"] != "draft" else 3,
        })

        first_author_id = author_id + 1
        pub_rows.append({
            "publication_id": pid, "access_status": 0,
            "date_published": sub.get("date_published") if published else None,
            "last_modified": sub["date_last_activity"],
    ...
    A(insert("submissions",
             ["submission_id", "context_id", "current_publication_id",
              ...
              "stage_id", "locale", "submission_progress"], sub_rows))
